...
## 9. prompts/review 配下のファイル

`prompts/review/` は、実装AIの提案や方針逸脱を第三者レビューするためのプロンプト集です。

- 実装を行うためのプロンプトではありません。
- 目的は、ユーザー方針に沿っているか、逸脱に合理性があるかを判定することです。
- 最終決定権はユーザーにあります。
- ユーザー方針を尊重しつつ、ユーザー側のリスクや考慮不足も明確に扱います。

- `review/01_third_party_review.md`
  実装AIの提案や逸脱が妥当かを第三者視点で審査するプロンプト。

- `review/02_policy_deviation_request.md`
  実装AIに、当初設計から外れる場合の変更申請を求めるプロンプト。

- `review/03_create_decision_log.md`
  方針逸脱レビューの判断ログを作成するプロンプト。


## 10. GitHub URLを直接読めない場合
