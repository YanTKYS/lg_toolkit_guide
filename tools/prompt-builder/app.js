(function () {
  ...
  function phaseText() {
    ...
    const map = {
      ...
      "設計文書と実装を作成": "案件に必要な文書を選び、実装、確認、テスト、文書整合まで一連で完了してください。",
      "既存ツールをレビュー": "既存ツールをガイドに照らして点検し、通常業務に影響する問題は可能な範囲で修正、テスト、文書整合まで行ってください。問題が再現しなければ無理に変更せず確認結果を記録してください。",
      "リリース前文書を整備": "既存実装を大きく変更せず、リリース前に必要なREADME、リリース前チェックリスト、テストシナリオ、運用引継ぎ文書、各種手順書を整備してください。",
      "既存ツールをガイド準拠に補完": "既存の設計・実装を活かし、必要な修正、確認、テスト、文書整合まで行ってください。文書や機能を機械的に追加しないでください。"
    };
    return map[phase] || "";
  }
